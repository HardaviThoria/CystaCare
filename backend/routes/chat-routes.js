...
chatRouter.post("/", async function(req,response,next){
    ...
    if (!process.env.OPENAI_API_KEY) {
      convoArr.push({question: query, reply: "OPENAI_API_KEY is not set. Please add it in backend/.env and restart the server."});
    ...
    try {
      ...
      console.log(err);
      convoArr.push({question: query, reply: "Chatbot is unavailable right now. Please verify OPENAI_API_KEY and try again."});
      return response.render("chat",{res: convoArr});
    }
}); 
