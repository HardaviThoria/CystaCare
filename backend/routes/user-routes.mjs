...
router.post("/signup", async(req,res) => {
      ...
      if (existingUser) {
        return res.render("signup", {existingUser: true});
      }
  ...
  router.post("/login", async(req,res) => {
    ...
    const { email, password } = req.body;
      ...
      if (!existingUser) {
         return res.render("login", {existingUser: false});
      }
      ...
      if (!isPasswordCorrect) {
        ...
        return res.status(400).render("login", {existingUser: "incorrect"});
      }
  ...
  router.get("/journal", auth, async(req,res) => {
    ...
    if(!Posts){
      return res.render("journal", {posts: []});
    }
